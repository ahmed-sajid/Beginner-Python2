...
#This function will jab at the player and provide unhelpful responses when they do not type in the proper response


def unhelpfulBot():
    
    listt = ['Are you even trying?',"Are you secretly an idiot?","bro you good??????",'I wish I could atleast say "nice try"...',"hahahahahaha","L + Ratio + Flop Era"]
    index = random.randint(0,len(listt)-1)
    wisecrack = listt[index]
    print(f'[Bot]\t{wisecrack}.')
    return

#This function will interject if the user types in certain commands to leave the program
# ('kbot shutdown' still ends it')

def lonelyBot():
    listt = ["Please don't go.","I am so lonely, please stay...","DON'T LEAVE ME","OK after one more joke.","PRETTY PRETTY PRETTY PLEASE LAST ONE!","You promised me you wouldn't ever leave :( ","I thought we could be friends forever..."]
    index = random.randint(0,len(listt)-1)
    wisecrack = listt[index]
    print(f'[Bot]\t{wisecrack}.')
    return
...
def answerCorrectly(question,expected_response):
    banned = ('quit end stop leave exit shutdown abbort bye ').split()
    you = ''
    x = -1
